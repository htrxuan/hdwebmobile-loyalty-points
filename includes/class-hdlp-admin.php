...
class HDLP_Admin
{
    ...
    public function register_hub_tabs($tabs)
    {
        $tabs['loyalty-points'] = array(
            'label'  => __('Loyalty Points', 'hdwebmobile-loyalty-points'),
            'order'  => 65,
    ...
    public function render_settings_page()
    {
        ...
        ?>
        <p><?php esc_html_e('Customers earn points on completed orders and redeem them for a discount at checkout. Balances are always computed from the ledger below -- never a value that can be set directly.', 'hdwebmobile-loyalty-points'); ?></p>
        ...
        <h2><?php esc_html_e('Manual Adjustment', 'hdwebmobile-loyalty-points'); ?></h2>
        <p><?php esc_html_e('Grant or deduct points for a specific customer, e.g. for customer service. This is logged in the ledger with your admin account attached.', 'hdwebmobile-loyalty-points'); ?></p>
        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
            ...
            <table class="form-table">
                <tr>
                    <th><label for="hdlp_user_email"><?php esc_html_e('Customer email', 'hdwebmobile-loyalty-points'); ?></label></th>
                    <td><input type="email" id="hdlp_user_email" name="hdlp_user_email" class="regular-text" required /></td>
                </tr>
                <tr>
                    <th><label for="hdlp_adjust_points"><?php esc_html_e('Points (use a negative number to deduct)', 'hdwebmobile-loyalty-points'); ?></label></th>
                    <td><input type="number" id="hdlp_adjust_points" name="hdlp_adjust_points" step="1" required /></td>
                </tr>
                <tr>
                    <th><label for="hdlp_adjust_note"><?php esc_html_e('Note', 'hdwebmobile-loyalty-points'); ?></label></th>
                    <td><input type="text" id="hdlp_adjust_note" name="hdlp_adjust_note" class="regular-text" placeholder="<?php esc_attr_e('e.g. Compensation for delayed order', 'hdwebmobile-loyalty-points'); ?>" /></td>
                </tr>
            </table>
            <?php submit_button(__('Apply Adjustment', 'hdwebmobile-loyalty-points')); ?>
        </form>

        <hr />

        <h2><?php esc_html_e('Recent Ledger Activity', 'hdwebmobile-loyalty-points'); ?></h2>
    ...
    private function render_recent_ledger()
    {
        ...
        if (empty($rows)) {
            echo '<p>' . esc_html__('No activity yet.', 'hdwebmobile-loyalty-points') . '</p>';
            return;
        }

        echo '<table class="widefat striped"><thead><tr>';
        echo '<th>' . esc_html__('Date', 'hdwebmobile-loyalty-points') . '</th>';
        echo '<th>' . esc_html__('Customer', 'hdwebmobile-loyalty-points') . '</th>';
        echo '<th>' . esc_html__('Points', 'hdwebmobile-loyalty-points') . '</th>';
        echo '<th>' . esc_html__('Type', 'hdwebmobile-loyalty-points') . '</th>';
        echo '<th>' . esc_html__('Note', 'hdwebmobile-loyalty-points') . '</th>';
        echo '</tr></thead><tbody>';
    ...
    public function handle_manual_adjustment()
    {
        if (!isset($_POST['hdlp_manual_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['hdlp_manual_nonce'])), 'hdlp_manual_adjustment')) {
            wp_die(esc_html__('Invalid request.', 'hdwebmobile-loyalty-points'));
        }

        if (!current_user_can('manage_woocommerce')) {
            wp_die(esc_html__('You do not have permission to do this.', 'hdwebmobile-loyalty-points'));
        }
        ...
        $note   = isset($_POST['hdlp_adjust_note']) ? sanitize_text_field(wp_unslash($_POST['hdlp_adjust_note'])) : __('Manual adjustment', 'hdwebmobile-loyalty-points');

        $user = get_user_by('email', $email);
        $redirect = admin_url('admin.php?page=hdwebmobile&tab=loyalty-points');

        if (!$user) {
            set_transient('hdlp_admin_notice_' . get_current_user_id(), array('type' => 'error', 'message' => __('No user found with that email.', 'hdwebmobile-loyalty-points')), 30);
            wp_safe_redirect($redirect);
            exit;
        }

        HDLP_Ledger::manual_adjustment($user->ID, $points, get_current_user_id(), $note);
        set_transient('hdlp_admin_notice_' . get_current_user_id(), array('type' => 'success', 'message' => __('Adjustment applied.', 'hdwebmobile-loyalty-points')), 30);
        wp_safe_redirect($redirect);
        exit;
    }
    ...
    public function page_init()
    {
        ...
        add_settings_section('hdlp_section_general', __('General', 'hdwebmobile-loyalty-points'), '__return_false', 'hdlp-settings');

        add_settings_field('enabled', __('Enable loyalty points', 'hdwebmobile-loyalty-points'), array($this, 'enabled_callback'), 'hdlp-settings', 'hdlp_section_general');
        add_settings_field('earn_status', __('Award points when order status becomes', 'hdwebmobile-loyalty-points'), array($this, 'earn_status_callback'), 'hdlp-settings', 'hdlp_section_general');
        add_settings_field('points_per_currency', __('Points earned per 1 currency unit spent', 'hdwebmobile-loyalty-points'), array($this, 'points_per_currency_callback'), 'hdlp-settings', 'hdlp_section_general');
        add_settings_field('points_redemption_rate', __('Points required per 1 currency unit of discount', 'hdwebmobile-loyalty-points'), array($this, 'points_redemption_rate_callback'), 'hdlp-settings', 'hdlp_section_general');
        add_settings_field('min_points_to_redeem', __('Minimum points required to redeem', 'hdwebmobile-loyalty-points'), array($this, 'min_points_callback'), 'hdlp-settings', 'hdlp_section_general');
        add_settings_field('max_redeem_percent', __('Maximum % of order redeemable with points', 'hdwebmobile-loyalty-points'), array($this, 'max_redeem_percent_callback'), 'hdlp-settings', 'hdlp_section_general');
    }
